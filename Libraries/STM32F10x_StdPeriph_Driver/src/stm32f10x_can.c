...
uint8_t CAN_Transmit(CAN_TypeDef * CANx, CanTxMsg * TxMessage)
{
	...
	if (transmit_mailbox != CAN_TxStatus_NoMailBox) {
		...
		CANx->sTxMailBox[transmit_mailbox].TDTR &=
		    (uint32_t) 0xFFFFFFF0;
...
uint8_t CAN_TransmitStatus(CAN_TypeDef * CANx, uint8_t TransmitMailbox)
{
	...
	case (CAN_TXMAILBOX_0):
		state =
		    CANx->TSR & (CAN_TSR_RQCP0 | CAN_TSR_TXOK0 | CAN_TSR_TME0);
		break;
	case (CAN_TXMAILBOX_1):
		state =
		    CANx->TSR & (CAN_TSR_RQCP1 | CAN_TSR_TXOK1 | CAN_TSR_TME1);
		break;
	case (CAN_TXMAILBOX_2):
		state =
		    CANx->TSR & (CAN_TSR_RQCP2 | CAN_TSR_TXOK2 | CAN_TSR_TME2);
		break;
	default:
...
void CAN_Receive(CAN_TypeDef * CANx, uint8_t FIFONumber, CanRxMsg * RxMessage)
{
	...
	/* Get the FMI */
	RxMessage->FMI =
	    (uint8_t) 0xFF & (CANx->sFIFOMailBox[FIFONumber].RDTR >> 8);
	/* Get the data field */
	RxMessage->Data[0] =
	    (uint8_t) 0xFF & CANx->sFIFOMailBox[FIFONumber].RDLR;
	RxMessage->Data[1] =
	...
	RxMessage->Data[4] =
	    (uint8_t) 0xFF & CANx->sFIFOMailBox[FIFONumber].RDHR;
	RxMessage->Data[5] =
...
uint8_t CAN_OperatingModeRequest(CAN_TypeDef * CANx, uint8_t CAN_OperatingMode)
{
	...
	if (CAN_OperatingMode == CAN_OperatingMode_Initialization) {
		...
		/* Wait the acknowledge */
		while (((CANx->MSR & CAN_MODE_MASK) != CAN_MSR_INAK)
		       && (timeout != 0)) {
			timeout--;
		}
		...
		/* Wait the acknowledge */
		while (((CANx->MSR & CAN_MODE_MASK) != CAN_MSR_SLAK)
		       && (timeout != 0)) {
			timeout--;
		}
...
FlagStatus CAN_GetFlagStatus(CAN_TypeDef * CANx, uint32_t CAN_FLAG)
{
	...
	if ((CAN_FLAG & CAN_FLAGS_ESR) != (uint32_t) RESET) {
		...
		/* Check the status of the specified CAN flag */
		if ((uint32_t) (CANx->RF1R & (CAN_FLAG & 0x000FFFFF)) !=
		    (uint32_t) RESET) {
			/* CAN_FLAG is set */
			bitstatus = SET;
		} else {
...
ITStatus CAN_GetITStatus(CAN_TypeDef * CANx, uint32_t CAN_IT)
{
	...
	if ((CANx->IER & CAN_IT) != RESET) {
		...
		case CAN_IT_TME:
			...
			itstatus =
			    CheckITStatus(CANx->TSR,
					  CAN_TSR_RQCP0 | CAN_TSR_RQCP1 |
					  CAN_TSR_RQCP2);
			break;
		case CAN_IT_FMP0:
