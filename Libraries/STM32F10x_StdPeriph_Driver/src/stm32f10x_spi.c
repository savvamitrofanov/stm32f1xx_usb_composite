...
/*----------------------- SPIx I2SCFGR & I2SPR Configuration -----------------*/
	...
	/* If the requested audio frequency is not the default, compute the prescaler */
		...
		if ((RCC->CFGR2 & tmp) != 0) {
			...
			sourceclock =
			    (uint32_t) (((RCC->CFGR2 & I2S_DIV_MASK) >> 4) + 1);

			/* Calculate the Source Clock frequency based on PLL3 and PREDIV2 values */
			sourceclock =
			    (uint32_t) ((HSE_Value / sourceclock) * tmp * 2);
		} else {
		...
		if (I2S_InitStruct->I2S_MCLKOutput == I2S_MCLKOutput_Enable) {
			...
			tmp =
			    (uint16_t) (((((sourceclock / 256) * 10) /
					  I2S_InitStruct->I2S_AudioFreq)) + 5);
		} else {
			/* MCLK output is disabled */
			tmp =
			    (uint16_t) (((((sourceclock / (32 * packetlength)) *
					   10) /
					  I2S_InitStruct->I2S_AudioFreq)) + 5);
		}
...
/**
  ...
  */
void
SPI_I2S_ITConfig(SPI_TypeDef * SPIx, uint8_t SPI_I2S_IT,
...
/**
  ...
  */
void
SPI_I2S_DMACmd(SPI_TypeDef * SPIx, uint16_t SPI_I2S_DMAReq,
	       FunctionalState NewState)
{
	/* Check the parameters */
	assert_param(IS_SPI_ALL_PERIPH(SPIx));
